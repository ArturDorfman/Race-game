document.addEventListener('DOMContentLoaded', () => {
	...
	// let playgroundBody = game.querySelector('#play-body');
	...
	function startGame() {
		...
		/* function drawInTable() {
			let tableLines = document.querySelectorAll('.table__line');
			// add values to the lines
			for (let i = 0; i < tableLines.length; i++) {
				let tableCells = tableLines[i].querySelectorAll('.table__cell');
				for (let j = 0; j < tableCells.length; j++) {
					tableCells[j].innerHTML = road[i][j];
				}
			}
			// draw a car
			for (let i = 0; i < tableLines.length; i++) {
				tableLines[i].querySelectorAll('.table__cell')[0].style.backgroundColor = null;
			}
			tableLines[spaceShip].querySelectorAll('.table__cell')[0].style.backgroundColor = 'red';
		} */
		...
		function drawCanvas() {
			...
			ctx.lineWidth = '2';
			ctx.setLineDash([20, 15]); // пунктирная линия длинна 20px, разрыв между линиями 15px
	...
	function results() {
		...
		let resultHits = gameResult.querySelector('#result-hits');
		let resultMisses = gameResult.querySelector('#result-misses');
